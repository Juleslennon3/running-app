...
export default function ProfileScreen() {
  ...
  async function toggleFollow(targetId: string) {
    ...
    setFollowingCount((prev) => prev + (currentlyFollowing ? -1 : 1))
  ...
  async function openFollowers() {
    ...
    if (!error && data) {
      setFollowersList(data.map((row: any) => ({ id: row.follower_id, username: row.profiles?.username ?? 'Unknown' })))
    }
  ...
  async function openFollowing() {
    ...
    if (!error && data) {
      setFollowingList(data.map((row: any) => ({ id: row.following_id, username: row.profiles?.username ?? 'Unknown' })))
    }
  ...
  return (
    <ScrollView style={styles.scrollView} contentContainerStyle={styles.container}>
      ...
      <UserListModal
        ...
        title="Followers"
        users={followersList}
        isFollowing={(id) => myFollowingIds.has(id)}
